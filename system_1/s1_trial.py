...
class System:
    ...
    def send_hash(self, sub_dir):
        ...
        for filename in filenames:
            if filename == "s1_trial.py" or filename == "file1.log":
                continue

            self.client.recv(1024)

            hash_value = self.hash_file(
                os.path.join(self.SERVER_DIR, sub_dir, filename)
            )
    ...
    def update_shared_dirs(self, sub_dir):
        dir_path, dir_names, filenames = list(
            os.walk(os.path.join(self.SERVER_DIR, sub_dir))
        )[0]
    ...
    def download_file(self, message):
        ...
        server_path = self.SERVER_DIR + messages[1]
        client_path = self.CLIENT_DIR + messages[1]

        self.client.send("1 " + client_path)
        ...
        if os.path.exists(server_path):
            hash_2 = self.hash_file(server_path)
            ...
            if hash_1 == hash_2:
                ...
                if val == "True":
                    if os.access(server_path, os.W_OK):
    ...
    def update_sharedfolders_r(self):
        while True:
            ...
            if messages[0] == "1":
                if messages[1] == "/s2_trial.py" or messages[1] == "/file2.log":
    ...
    def run(self):
        ...
        f = open(self.SERVER_DIR + "/" + "file1.log", "a+")
                            ...
                            if l == "Over":
                                ...
                                if commands[1] == "longlist":
                                    ...
                                    if length < 4:
                                        print("INVALID COMMAND")
                                        break
                                    start_time = float(commands[2])
                                    end_time = float(commands[3])

                                    if start_time <= mtime and mtime <= end_time:
                                        print(
                                            "FILE-",
                                            ls[0],
                                            ", SIZE-",
                    ...
                    if messages[1] == "UDP":
                        ...
                        if os.path.exists(self.SERVER_DIR + "/" + filename):
                            ...
                            if mess == "SEND":
                                ...
                                l = f.read(1024)
                                while l:
                                    clientSocket.sendto(l, (self.HOST, 50002))
                                    l = f.read(1024)
                                clientSocket.sendto("Over", (self.HOST, 50002))
                                clientSocket.close()
